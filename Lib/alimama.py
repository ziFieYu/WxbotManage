...
class alimama:
    ...
    def __init__(self,siteid=None,zoneid=None,sign_account=None,login_type='local'):
        ...
        self.s.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.63 Safari/537.36'})
        ...
        if result:
            ...
            if tkSpecialCampaignIdRateMap != None:
                ...
                for x in tkSpecialCampaignIdRateMap:
                    ...
                    if int(x) > 0 :
                        if  float(tkSpecialCampaignIdRateMap[x]) >= float(item_info['tkRate']) and float(tkSpecialCampaignIdRateMap[x]) >= float(temp_rate):
                            temp_cid = x
                ...
                if temp_cid != '':
                    ...
                    for x in exsitApplyList:
                        if str(x['campaignId']) == str(temp_cid):
                            print u'[INFO] %s已经申请了高佣金活动了,活动ID：%s-->%s'%(item_info['auctionId'],str(temp_cid),temp_rate)
                            is_apply = True
                            break
                    #h获取高佣金通道信息，并进行申请
                    if is_apply == False:
                        for x in campaignList:
                            if str(x['campaignId']) == str(temp_cid):
                                ...
                                if result:
                                    print u'[INFO] %s成功申请高佣金通道,原佣金%s,现佣金%s'%(item_info['auctionId'],str(item_info['tkRate']),temp_rate)
                                else:
            ...
            if item_info['eventRate']  != None:
                if float(item_info['eventRate']) > float(temp_rate):
                    print  u'[INFO] %s的鹊桥高佣金推广比例（%s）高于定向推广（%s）'%(item_info['auctionId'],item_info['eventRate'],temp_rate) 
                    return 1   
            return 0
    ...
    def get_coupons_info(self,sid,cid,coupon_url=None):
        ...
        if coupon_url == None:
            ...
            url = coupon_url
        try:
            content = self.s.get(url,headers={'User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1'}).content.replace('\t','').replace('\n','').replace(' ','')
            content = re.findall(r'<divclass="coupon-info">(.*?)</div>',content)[0]
            result =  re.findall(r'<dl><dt>(\d+?)元优惠券</dt><dd>剩<spanclass="rest">(\d+?)</span>张（已领用<spanclass="count">(\d+?)</span>张）</dd><dd>单笔满(\d+?)元可用，每人限领(\d+?)张</dd><dd>有效期:(.*?)至(.*?)</dd></dl>',content)
    ...
    def match_best_coupon(self,item_info,coupons_info):
        ...
        for x in coupons_info:
            print u'商品折扣价:%d，代金券满%d可减%d元'%(item_info['zkPrice'],int(x['activity_info'][3]),int(x['activity_info'][0]))
            if int(x['activity_info'][3]) <= item_info['zkPrice'] and int(x['activity_info'][0])> best_coupon['chae']:
                ...
                find = True
        if find:
            print u'商品折扣价:%d，合适的代金券->满%d可减%d元'%(item_info['zkPrice'],int(x['activity_info'][3]),int(x['activity_info'][0]))
    ...
    def create_auction_code(self,iid,siteid,adzoneid):
        ...
        """
        url = u'http://pub.alimama.com/common/code/getAuctionCode.json?auctionid='+str(iid)+'&adzoneid'+str(adzoneid)+'&siteid='+str(siteid)
        try:
            c = self.s.get(url).json()
            return True,c
        except Exception,e:
    ...
    def create_auction_code_with_tkl(self,iid,siteid,adzoneid,item_info=None):
        ...
        if result ==0:
            url = u'http://pub.alimama.com/common/code/getAuctionCode.json?auctionid='+str(iid)+'&adzoneid='+str(adzoneid)+'&siteid='+str(siteid)+'&scenes=1&pvid='+str(self.pvid)+'&t='+str(int(time.time()))
        else:
            url = u'http://pub.alimama.com/common/code/getAuctionCode.json?auctionid='+str(iid)+'&adzoneid='+str(adzoneid)+'&siteid='+str(siteid)+'&scenes=3&channel=tk_qqhd&pvid='+str(self.pvid)+'&t='+str(int(time.time()))    
        try:
            c = self.s.get(url).json()
            return True,c
        except Exception,e:
    ...
    def search_item_info_by_iid(self,iid):
        ...
        根据淘宝iid生成查询相关信息
        """
        url = u"http://pub.alimama.com/items/search.json?q=https://detail.tmall.com/item.htm?id=%s&_t=%s&auctionTag=&perPageSize=40"%(str(iid),str(int(time.time())))    
        try:
            c = self.s.get(url).json()
    ...
    def search_item_info_by_key(self,key):
        ...
        根据关键词生成查询相关信息
        """
        url = u"http://pub.alimama.com/items/search.json?q=%s&_t=%s&auctionTag=&perPageSize=40"%(key,str(int(time.time())))   
        try:
            c = self.s.get(url).json()
            self.pvid = c['info']['pvid']
            return True,c
    ...
    def search_item_info_by_key_use_queqiao(self,key):
        ...
        鹊桥接口——根据关键词生成查询相关信息
        """
        url = u"http://pub.alimama.com/items/channel/qqhd.json?q=%s&channel=qqhd&toPage=1&sortType=5&startBiz30day=10&perPageSize=50&shopTag=&t=%s"%(key,str(int(time.time())))
    ...
    def search_item_info_by_iid_use_queqiao(self,iid):
        ...
        鹊桥接口——根据iid生成查询相关信息
        """
        url = u"http://pub.alimama.com/items/channel/qqhd.json?q=https://item.taobao.com/item.htm?id="+str(iid)+"&channel=qqhd&toPage=1&sortType=1&startBiz30day=10&perPageSize=50&shopTag=&t="+str(int(time.time()))    
        try:
            c = self.s.get(url).json()
            self.pvid = c['info']['pvid']
            return True,c
    ...
    def check_login_status(self):
        ...
        try:
            ...
            try:
                self.account_info  = c
                self._tb_token_ = re.findall(r"<input name='_tb_token_' type='hidden' value='(.*?)'>",self.s.get('http://www.alimama.com/index.htm').content)[0]
    ...
    def auto_login(self):
        ...
        try:
            ...
            if userid != '' :
                ...
                r = self.s.get('https://www.alimama.com/membersvc/my.htm?domain=taobao&service=user_on_taobao&sign_account='+self.sign_account)
                self.s.get(r.url)
    ...
    def login(self,send_msg=None,target_id=None):
        while 1:
            ...
            #获取二维码扫描状态的url
            url = u"https://qrlogin.taobao.com/qrcodelogin/qrcodeLoginCheck.do?lgToken="+str(lgToken)+"&defaulturl=http%3A%2F%2Fwww.alimama.com&_ksTS="+login_time
            while 1:
